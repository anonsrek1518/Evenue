...
<div class="container-fluid mt-3 pt-2">
    ...
    <div class="row-items">
        <div class="col-lg-12">
            <div class="row">
                <?php
                $rtl ='rtl'; // Initialize direction variable
                $ci= 0; // Initialize counter variable
                // Retrieve venue data from database
                $venue = $conn->query("SELECT * from venue order by rand()");
                // Loop through each venue data
                while($row = $venue->fetch_assoc()):
                    $ci++;
                    // Set direction based on counter value
                    if($ci < 3){
                        $rtl = '';
                    } else {
                        $rtl = 'rtl';
                    }
                    // Reset counter after every 4 venues
                    if($ci == 4){
                        $ci = 0;
                    }
                ?>
